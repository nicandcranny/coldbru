...
import { IconVariable, IconSettings, IconRun, IconFolder, IconDatabase, IconWorld, IconHome, IconFileCode, IconGitBranch, IconBox } from '@tabler/icons';
  ...
  const getTabInfo = (type, tabName) => {
    switch (type) {
      case 'collection-settings': {
        return (
          <>
            <IconBox size={14} strokeWidth={1.5} className="special-tab-icon flex-shrink-0" />
            <span className="ml-1 tab-name">{tabName || 'Collection'}</span>
          </>
        );
      }
      ...
      case 'environment-settings': {
        return (
          <>
            ...
            <span className="ml-1 tab-name">{tabName || 'Environments'}</span>
          </>
        );
      }
      ...
      case 'git-diff': {
        return (
          <>
            ...
            <span className="ml-1 tab-name">{tabName || 'Diff'}</span>
          </>
        );
      }
    }
