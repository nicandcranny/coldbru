...
const isValidCollectionDirectory = (dirPath) => {
  ...
  return Boolean(getCollectionConfigPath(dirPath)) || fs.existsSync(opencollectionYmlPath);
};
...
const getCollectionFormat = (collectionPath) => {
  ...
  if (getCollectionConfigPath(collectionPath)) {
    return 'bru';
  }
...
const isBrunoConfigFile = (pathname, collectionPath) => {
  ...
  return path.normalize(dirname) === path.normalize(collectionPath) && COLLECTION_CONFIG_FILENAMES.includes(basename);
};
    ...
    if (files && files.length) {
      files.forEach((file) => {
        ...
        if (stat.isDirectory()) {
          ...
          scanDir(fullPath);
        } else if (COLLECTION_CONFIG_FILENAMES.includes(file)) {
