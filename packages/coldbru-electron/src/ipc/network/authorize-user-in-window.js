...
const authorizeUserInWindow = ({ authorizeUrl, callbackUrl, session, additionalHeaders = {}, grantType = 'authorization_code' }) => {
  return new Promise(async (resolve, reject) => {
    ...
    window.webContents.on('certificate-error', (event, url, error, certificate, callback) => {
      ...
      if (!shouldAllow) {
        console.error(`ColdBru OAuth: SSL Certificate verification failed for ${url}. Error: ${error}`);
        console.error('ColdBru OAuth: Disable "SSL/TLS Certificate Verification" in settings to proceed with OAuth flows that use self-signed certificates.');
      }
