...
const { openApiToBruno } = require('@coldbru/converters');
const { writeFile, sanitizeName, getCollectionFormat, getCollectionConfigPath } = require('../utils/filesystem');
...
const loadBrunoConfig = (collectionPath) => {
  ...
  if (format === 'yml') {
    ...
    const brunoJsonPath = getCollectionConfigPath(collectionPath);
    if (!brunoJsonPath || !fs.existsSync(brunoJsonPath)) {
      throw new Error('coldbru.json or bruno.json not found');
    }
...
/**
 * Save bruno config to disk (coldbru.json or opencollection.yml).
 */
const saveBrunoConfig = async (collectionPath, format, brunoConfig, collectionRoot) => {
  if (format === 'yml') {
    ...
    const brunoJsonPath = getCollectionConfigPath(collectionPath) || path.join(collectionPath, 'coldbru.json');
