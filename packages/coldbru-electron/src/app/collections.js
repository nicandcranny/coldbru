...
// todo: coldbru.json config schema validation errors must be propagated to the UI
...
const readConfigFile = async (pathname) => {
  try {
    ...
    return Promise.reject(new Error(`Unable to parse json in coldbru.json in ${pathname}`));
  }
};
...
const validateSchema = async (config) => {
  try {
    ...
    return Promise.reject(new Error('coldbru.json format is invalid in ' + config?.name));
  }
};
...
const getCollectionConfigFile = async (pathname) => {
  ...
  // Fall back to JSON collection config.
  const configFilePath = getCollectionConfigPath(pathname);
  if (!configFilePath) {
    throw new Error(`The collection is not valid (neither coldbru.json, bruno.json, nor opencollection.yml found)`);
  }
...
const openCollection = async (win, watcher, collectionPath, options = {}) => {
  ...
  if (watcher.hasWatcher(collectionPath)) {
    try {
      ...
      brunoConfig = await transformColdBruConfigAfterRead(brunoConfig, collectionPath);
