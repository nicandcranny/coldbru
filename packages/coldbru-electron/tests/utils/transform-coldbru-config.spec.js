const { transformColdBruConfigBeforeSave, transformColdBruConfigAfterRead } = require('../../src/utils/transformColdBruConfig');

describe('BrunoConfig Proxy Transform', () => {
  describe('transformColdBruConfigAfterRead - Migration from old to new format', () => {
    describe('Old Format: enabled (true | false | "global")', () => {
      test('should migrate enabled: true to disabled: false, inherit: false', async () => {
        ...
        const result = await transformColdBruConfigAfterRead(oldConfig, '/test/path');
    ...
    describe('New Format (no migration)', () => {
      test('should not modify new format with inherit: false', async () => {
        ...
        const result = await transformColdBruConfigAfterRead(newConfig, '/test/path');
    ...
    describe('Edge Cases', () => {
      test('should handle missing proxy config', async () => {
        ...
        const result = await transformColdBruConfigAfterRead(config, '/test/path');
  ...
  describe('transformColdBruConfigBeforeSave - Cleanup optional fields', () => {
    test('should remove disabled: false from proxy config', () => {
      ...
      const result = transformColdBruConfigBeforeSave(config);
  ...
  describe('Round-trip transformation', () => {
    test('should handle read -> save -> read cycle for old format', async () => {
      ...
      // Read (migrate)
      const afterRead = await transformColdBruConfigAfterRead(oldConfig, '/test/path');

      // Save (cleanup)
      const beforeSave = transformColdBruConfigBeforeSave(afterRead);

      // Read again (should not change)
      const afterSecondRead = await transformColdBruConfigAfterRead(beforeSave, '/test/path');
    ...
    test('should handle read -> save -> read cycle for new format', async () => {
      ...
      // Read (no migration)
      const afterRead = await transformColdBruConfigAfterRead(newConfig, '/test/path');
