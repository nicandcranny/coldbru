...
import { interpolate, mockDataFunctions, timeBasedDynamicVars } from '@coldbru/common';
...
export const renderVarInfo = (token, options) => {
  ...
  if (!isReadOnly && scopeInfo) {
    ...
    const cmEditor = CodeMirror(editorContainer, {
      ...
      lineWrapping: true,
      lineNumbers: false,
      coldbruVarInfo: false, // Disable tooltips within the editor to prevent recursion
      scrollbarStyle: null,
...
if (!SERVER_RENDERED) {
  ...
  CodeMirror.defineOption('coldbruVarInfo', false, function (cm, options, old) {
    if (old && old !== CodeMirror.Init) {
      const oldOnMouseOver = cm.state.coldbruVarInfo.onMouseOver;
      CodeMirror.off(cm.getWrapperElement(), 'mouseover', oldOnMouseOver);
      clearTimeout(cm.state.coldbruVarInfo.hoverTimeout);
      delete cm.state.coldbruVarInfo;
    }

    if (options) {
      const state = (cm.state.coldbruVarInfo = createState(options));
  ...
  function getHoverTime(cm) {
    const options = cm.state.coldbruVarInfo.options;
    return (options && options.hoverTime) || 50;
  }

  function onMouseOver(cm, e) {
    const state = cm.state.coldbruVarInfo;
        ...
        if (start >= 0 && line.substring(start, start + 2) === '{{') {
          ...
          if (end <= line.length && line.substring(end - 2, end) === '}}') {
            ...
            if (inner) {
              ...
              const coldbruVarInfo = renderVarInfo(token, options);
              if (coldbruVarInfo) {
                showPopup(cm, box, coldbruVarInfo);
  ...
  function showPopup(cm, box, coldbruVarInfo) {
    ...
    popup.className = 'CodeMirror-coldbruVarInfo';
    popup.appendChild(coldbruVarInfo);
