...
const useIpcEvents = () => {
  ...
  useEffect(() => {
    ...
    const removeApiSpecTreeUpdateListener = ipcRenderer.on(
    ...
    const removeOpenWorkspaceListener = ipcRenderer.on(
      ...
      (workspacePath, workspaceUid, workspaceConfig) => {
        dispatch(
          workspaceOpenedEvent(workspacePath, workspaceUid, workspaceConfig)
        );
      }
    );
    ...
    const removePersistentEnvVariablesUpdateListener = ipcRenderer.on(
    ...
    const removeWorkspaceDotEnvUpdatesListener = ipcRenderer.on(
      'main:workspace-dotenv-update',
      (val) => {
        dispatch(workspaceDotEnvUpdateEvent(val));
        dispatch(
          workspaceEnvUpdateEvent({
            processEnvVariables: val.processEnvVariables
          })
        );
      }
    );

    const removeDotEnvFileUpdateListener = ipcRenderer.on(
      ...
      (val) => {
        ...
        if (type === 'collection' && collectionUid) {
          ...
          if (filename === '.env') {
            dispatch(
              processEnvUpdateEvent({ collectionUid, processEnvVariables })
            );
          }
    ...
    const removeSystemResourcesListener = ipcRenderer.on(
    ...
    const removeCollectionOauth2CredentialsClearListener = ipcRenderer.on(
    ...
    const removeHttpStreamNewDataListener = ipcRenderer.on(
    ...
    const removeHttpStreamEndListener = ipcRenderer.on(
