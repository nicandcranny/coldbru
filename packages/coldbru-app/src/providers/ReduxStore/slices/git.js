...
import { clearGlobalEnvironmentDraft } from './global-environments';

const initialState = {
  collectionStates: {}
};

const operationsToPreserveDuringRefresh = new Set([
  'commit',
  'sync',
  'push',
  'pull',
...
export const refreshCollectionGitStatus
  = (collectionUid, options = {}) =>
    async (dispatch, getState) => {
      ...
      try {
        ...
        const data = await window.ipcRenderer.invoke(
          'renderer:get-collection-git-status',
          {
        ...
        dispatch(
          setCollectionGitState({
            ...
            data: {
              ...
              operation: shouldPreserveOperation
                ? latestGitState.operation
                : null,
...
export const stageGitFiles
  = (collectionUid, filePaths) => async (dispatch, getState) => {
    if (!filePaths?.length) {
      return;
    }
    ...
    return runGitOperation({
      ...
      operation: 'stage',
      action: (target) =>
        window.ipcRenderer.invoke('renderer:stage-git-files', {
          collectionPath: target.path,
          filePaths
        })
    });
...
export const unstageGitFiles
  = (collectionUid, filePaths) => async (dispatch, getState) => {
    ...
    return runGitOperation({
      ...
      operation: 'unstage',
      action: (target) =>
        window.ipcRenderer.invoke('renderer:unstage-git-files', {
          collectionPath: target.path,
          filePaths
        })
    });
...
export const revertGitFiles
  = (collectionUid, filePaths) => async (dispatch, getState) => {
    ...
    const draftEnvironment = state.globalEnvironments?.globalEnvironments?.find(
      (environment) => environment.uid === draft?.environmentUid
    );
    const draftPath = draftEnvironment
      ? `environments/${draftEnvironment.name}.yml`
      : null;
    const revertsDraft
      = draftPath
        && filePaths.some(
          (filePath) =>
            filePath.replace(/\\/g, '/').replace(/^\.\//, '') === draftPath
        );
    ...
    const result = await runGitOperation({
      ...
      successMessage:
        filePaths.length === 1
          ? 'Changes reverted'
          : 'All selected changes reverted',
      action: (target) =>
        window.ipcRenderer.invoke('renderer:revert-git-files', {
          collectionPath: target.path,
          filePaths
        })
    });
...
export const commitGitChanges
  = (collectionUid, message) => async (dispatch, getState) => {
    ...
    return runGitOperation({
      ...
      action: (target) =>
        window.ipcRenderer.invoke('renderer:commit-git-changes', {
          collectionPath: target.path,
...
export const initializeGitRepository
  = (collectionUid) => async (dispatch, getState) => {
    return runGitOperation({
      dispatch,
      getState,
      collectionUid,
      ...
      action: (target) =>
        window.ipcRenderer.invoke('renderer:init-git-repository', {
          collectionPath: target.path
        })
    });
  };

export const addGitRemote
  = (collectionUid, { remoteName, remoteUrl }) =>
    async (dispatch, getState) => {
      ...
      return runGitOperation({
        ...
        action: (target) =>
          window.ipcRenderer.invoke('renderer:add-git-remote', {
            collectionPath: target.path,
...
export const pushGitChanges = (collectionUid) => async (dispatch, getState) => {
  const { remoteName, remoteBranch } = getRemoteDetails(
    getState(),
    collectionUid
  );
  ...
  return runGitOperation({
    ...
    action: (target) =>
      window.ipcRenderer.invoke('renderer:push-git-changes', {
        collectionPath: target.path,
        remoteName,
        remoteBranch
      })
...
export const pullGitChanges = (collectionUid) => async (dispatch, getState) => {
  ...
  return runGitOperation({
    ...
    action: (target) =>
      window.ipcRenderer.invoke('renderer:pull-git-changes', {
        collectionPath: target.path,
        remoteName,
        remoteBranch,
        strategy: '--no-rebase'
