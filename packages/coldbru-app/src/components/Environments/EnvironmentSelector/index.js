...
const DropdownTrigger = React.forwardRef(
  ({ collectionEnv, globalEnv, showCollectionEnv }, ref) => {
    const hasNoVisibleEnvironments = showCollectionEnv
...
const EnvironmentSelector = ({ collection, showCollectionEnv = true }) => {
  ...
  const [activeTab, setActiveTab] = useState(
    showCollectionEnv ? 'collection' : 'global'
  );
  ...
  const activeCollectionEnvironment = activeEnvironmentUid
    ...
    : null;
  const visibleTabs = showCollectionEnv
    ? TABS
    : TABS.filter((tab) => tab.id === 'global');

  const dropdownWidth = useMemo(
    () =>
      calculateDropdownWidth(
        showCollectionEnv ? environments : [],
        globalEnvironments
      ),
  ...
  const visibleEnvironments = useMemo(() => {
    ...
    return currentEnvironments.filter((env) =>
  ...
  useEffect(() => {
    if (
      (!showCollectionEnv || !safeCollection.uid)
  ...
  useEffect(() => {
    const handleOpenEnvironmentSelector = (event) => {
      ...
      if (
        ...
        toast.error(
          'Collection environments are not available in current view'
        );
        return;
      }
  ...
  const handleEnvironmentSelect = (environment) => {
    ...
    dispatch(action)
      .then(() => {
        toast.success(
          environment
            ? `Environment changed to ${environment.name}`
            : 'No Environments are active now'
        );
        hideDropdown();
      })
      .catch(() => {
  ...
  const openGlobalEnvironmentSettingsTab = () => {
    ...
    const existingTab = find(
      tabs,
      (tab) => tab.uid === globalEnvironmentTabUid
    );
