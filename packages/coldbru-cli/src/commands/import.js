...
const { openApiToBruno, wsdlToBruno } = require('@coldbru/converters');
...
const handler = async (argv) => {
  try {
    ...
    if (type === 'openapi') {
      ...
      console.log(chalk.yellow('Converting OpenAPI specification to ColdBru format...'));

      // Convert OpenAPI to ColdBru format
      ...
      console.log(chalk.yellow('Converting WSDL to ColdBru format...'));

      // Convert WSDL to ColdBru format
    ...
    if (outputFile) {
      ...
      console.log(chalk.green(`ColdBru collection saved as JSON to ${outputPath}`));
    } else if (output) {
      ...
      console.log(chalk.green(`ColdBru collection created at ${outputDir}`));
    }
  } catch (error) {
