...
const { parseRequest, parseCollection, parseFolder, stringifyCollection, stringifyFolder, stringifyEnvironment, stringifyRequest } = require('@coldbru/filestore');
...
const COLLECTION_CONFIG_FILENAMES = ['coldbru.json', 'bruno.json'];

const getCollectionConfigPath = (collectionPath) => {
  for (const filename of COLLECTION_CONFIG_FILENAMES) {
    const configPath = path.join(collectionPath, filename);
...
const getCollectionFormat = (collectionPath) => {
  ...
  if (getCollectionConfigPath(collectionPath)) return 'bru';
  return null;
};
...
const getCollectionConfig = (collectionPath, format) => {
  ...
  const configPath = getCollectionConfigPath(collectionPath);
  const brunoConfig = JSON.parse(fs.readFileSync(configPath, 'utf8'));
...
const createCollectionFromBrunoObject = async (collection, dirPath, options = {}) => {
  ...
  if (format === 'bru') {
    fs.writeFileSync(
      path.join(dirPath, 'coldbru.json'),
